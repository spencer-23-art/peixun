...
(function () {
  ...
  function recordCard(r) {
    ...
    var ageWarning = r.gender === '女' ? Number(r.age) >= 55 : Number(r.age) >= 60;
    var ageHtml = (ageWarning ? '<span style="color:#ff0000;font-weight:bold;">' : '') + esc(r.age || '--') + '岁' + (ageWarning ? '</span>' : '');
    return '<article class="record-card ' + (downloaded ? 'is-downloaded' : '') + ' ' + (checked ? 'is-selected' : '') + '"><div class="record-head"><input class="record-select" type="checkbox" ' + (checked ? 'checked' : '') + ' aria-label="选择' + esc(r.name) + '" onchange="mobileAdminToggleRecord(' + Number(r.id) + ',this.checked)"><div class="person-line"><div class="person">' +
      ...
      '<div><div class="person-name">' + esc(r.name) + '</div><div class="person-meta">' + esc(r.gender || '--') + ' · ' + ageHtml + ' · ' + esc(r.education || '--') + '</div></div></div><span class="state-pill ' + (downloaded ? '' : 'warning') + '">' + (downloaded ? '已下载' : '待下载') + '</span></div></div>' +
      '<div class="record-details"><div><span>工作单位</span><strong>' + esc(r.company || '暂无单位') + '</strong></div><div><span>联系电话</span><strong>' + esc(r.phone || '--') + '</strong></div><div><span>身份证号</span><strong>' + esc(r.id_card || '--') + '</strong></div><div><span>录入人</span><strong>' + esc(r.recorder_name || '系统管理员') + '</strong></div><div><span>岗位 / 区域</span><strong>' + esc(r.job || '--') + ' · ' + esc(r.region_auth || '--') + '</strong></div>' +
